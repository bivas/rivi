...
func assertClientConfig(t *testing.T, config client.ClientConfig) {
	assert.Equal(t, config.GetOAuthToken(), "github-token", "oauth token")
	assert.Equal(t, config.GetSecret(), "github-secret", "secret")
}

func assertRoles(t *testing.T, configuration Configuration) {
...
func assertRules(t *testing.T, configuration Configuration) {
...
func TestActionConfigBuilder(t *testing.T) {
